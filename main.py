...
def update_product(pid: str, p: Product, admin=Depends(verify_admin)):
    data = p.dict()
    data.pop("id", None)
    res = supabase.table("products").update(data).eq("id", pid).execute()
    if not res.data:
...
def update_boost(bid: str, b: Boost, admin=Depends(verify_admin)):
    data = b.dict()
    data.pop("id", None)
    res = supabase.table("boosts").update(data).eq("id", bid).execute()
    if not res.data:
...
def update_load(lid: str, l: Load, admin=Depends(verify_admin)):
    data = l.dict()
    data.pop("id", None)
    res = supabase.table("loads").update(data).eq("id", lid).execute()
    if not res.data:
...
def get_users(admin=Depends(verify_admin)):
    res = supabase.table("users").select("id,username,name,joined,login_count,last_login").execute()
    return res.data
...
def login_user(body: dict):
    ...
    import hashlib
    pass_hash = hashlib.sha256(password.encode()).hexdigest()
    res = supabase.table("users").select("*").eq("username", username).eq("pass_hash", pass_hash).execute()
    if not res.data:
...
def change_password(username: str, body: dict):
    ...
    res = supabase.table("users").select("id").eq("username", username).eq("pass_hash", old_hash).execute()
    if not res.data:
        raise HTTPException(status_code=401, detail="Current password is incorrect")
    new_hash = hashlib.sha256(new_pw.encode()).hexdigest()
    supabase.table("users").update({"pass_hash": new_hash}).eq("username", username).execute()
    return {"ok": True}
